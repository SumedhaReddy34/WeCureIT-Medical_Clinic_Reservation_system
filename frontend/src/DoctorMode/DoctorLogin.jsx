...
function DoctorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/doctorlogin/authenticate', {
        email,
        password,
      });
      ...
      if (response.status === 200) {
        const { token, doctorId } = response.data; // Extract token and doctorId from response
        sessionStorage.setItem('authToken', token); // Store the token in sessionStorage
        sessionStorage.setItem('DoctorId', doctorId); // Store the doctorId
      ...
      <header className="w-full px-4 py-3 bg-white border-b border-gray-300 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-indigo-800">WeCureIt</h1>
      ...
      <div className="flex-grow flex items-center justify-center">
        <div className="bg-white border rounded-lg shadow-lg w-[400px] p-6">
          <h2 className="text-lg font-medium text-center text-gray-800 mb-6">
            <span className="text-indigo-800 font-bold">Doctor</span> Login
          </h2>
          <form onSubmit={handleLogin}>
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring focus:ring-indigo-300"
            >
              Login
            </button>
          </form>
